		...
		int filedLenght = valueField.length;
		 for (int j = 0; j < filedLenght; j++) {
			 try {
				 ...
				 ExceptionDispose.catchExceptionAndSaveToDatabase("StargazerSystem Error Report : "+" 输入参数类型不合法");
		     } catch (IllegalAccessException e) {
				ExceptionDispose.catchExceptionAndSaveToDatabase("StargazerSystem Error Report : "+" 目标类没有访问权限");
			}
		 }
	}
